...
static void sensor_dump(const void *data); // sensor_t の内容を標準出力にダンプするコールバック

// --- 関数定義（概要→詳細の順） ---

int main(int argc, char *argv[])
{
    size_t shm_size = SHM_CAPACITY * sizeof(sensor_t); // 共有メモリの総バイトサイズ
...
{
    const sensor_t *s = (const sensor_t *)data; // ダンプ対象の構造体ポインタ
    printf("sensor_t {\n");
    printf("  location    = \"%s\"\n", s->location);
